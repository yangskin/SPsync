...
def create_material(path:str, bco_path:str, es_path:str, mra_path:str, n_path:str, udmi:bool, material_type:str, emissive_type:bool)->unreal.Material:
    ...
    if asset_library.do_assets_exist([path]):
        ...
        if not material:
            unreal.log_error(f"[SPsync] Failed to create material: {path}")
            return None
        try:
            ...
            if emissive_scattering_texture:
            ...
            if metallic_roughness_ao_texture:
            ...
            unreal.log_error(f"[SPsync] Error setting up material {path}: {e}")
            return material